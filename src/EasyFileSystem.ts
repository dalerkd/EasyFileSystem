...
class InodeManager {
    //直接用json来处理 inode
    static toBuffer(inodes: IF_Inode[]): Buffer {
        return Buffer.from(JSON.stringify(inodes));
    }
    static fromBuffer(buffer: Buffer): Array<IF_Inode> {
...
export class DirectoryManager {
    ...
    LoadFromBuffer(bufferUint8: Uint8Array) {
        ...
        //
        let buffer: ArrayBuffer = bufferUint8.buffer.slice(
            bufferUint8.byteOffset,
            bufferUint8.byteOffset + bufferUint8.byteLength
        );
        ...
        for (let n = 0; n < itemNumber; n++) {
            ...
            offset += 4;
            const u8aStr: Uint8Array = new Uint8Array(
...
export default class EasyFileSystem {
    ...
    static ROOT_PATH_INODE_NUMBER = 0
    private initMetaEntry() {
        let directory = new DirectoryManager();
        const inode_index = this.m_nodeArray.length ?
            EasyFileSystem.ROOT_PATH_INODE_NUMBER : INVALID_INODE_INDEX
    ...
    private m_read_only_mode: boolean = false;
    /**
     * 将数据压入 Disk  和 Inode
     ...
     */
    private FixDirInode(
        fatherInodeIndex: number,
        childItemFileName: string,
        childItemType: InodeType,
        childItemIndex: number
    ) {
        if (childItemFileName == "/") {
            this.m_meta_entry.replaceInode(
                childItemFileName,
                childItemType,
                childItemIndex
            );
        ...
        let buffer = this.m_diskBuffer.slice(
            node.offset,
        ...
        this.m_diskBuffer = Buffer.concat([
            preDiskBuffer,
            middleDiskBuffer,
            tailDiskBuffer,
        ]);
    ...
    private GetDataByInode(indexByInode: number): Uint8Array {
        ...
        if (indexByInode >= len) {
            throw `超范围: 获取数据通过节点索引操作,发现节点索引: ${indexByInode}超过现在索引最大值: ${
            len - 1
            }`;
        }
    ...
    CheckNoReadOnlyMode() {
        if (this.m_read_only_mode) {
            throw ('只读模式-设置的保存路径存在文件')
        }
    }
    async PackDirctory(dirPath: string) {
        this.CheckNoReadOnlyMode()
    ...
    private executeExplorer(cbDir: (arg0: string, arg1: string) => boolean, cbFile: (arg0: string, arg1: string, arg2: Buffer) => boolean, virtualPath = '/') {
        if (this.hasExistDirectory(virtualPath)) {
            let dm = this.DirDirectory(virtualPath)
            for (let i = 0; i < dm.itemArray.length; i++) {
                let item = dm.itemArray[i]
                let name = item.name
                let type = item.type
                if (type === InodeType.File) {
                    if (cbFile(virtualPath, name, Buffer.from(this.ReadFile(virtualPath + name)))) return
                } else if (type === InodeType.Directory) {
                    if (cbDir(virtualPath, name)) return
                    if (name == '.' || name == '..') {
                        continue
                    }
                    this.executeExplorer(cbDir, cbFile, virtualPath + name + '/')
                } else {
                    throw (`意料外的类型: ${type},在 ${virtualPath} 下,name: ${name}`)
                }
            }
        }
    }
    static TestMySelf() {
        ...
        ASSERT(
            efs.DirDirectory("/").itemArray.length == 2,
            "根目录下原始文件夹数量错误"
        );
        ASSERT(efs.hasExistDirectory("/") == true, "不存在预期的 根文件夹");
        ASSERT(
            efs.hasExistDirectory("/./") == true,
            "不存在预期的.文件夹在根目录下"
        );
        ASSERT(
            efs.hasExistDirectory("/./d/") == false,
            "对不存在的d 没有给出不存在的反馈"
        );
        cl("测试 文件夹存在性 相关测试 √");
        ...
        ASSERT(
            Utf8ArrayToStr(efs.ReadFile("/./d/f六号文档.txt")) == "1234\n中5678"
        );
        cl("中文文件名及中文读写文件测试    √");
